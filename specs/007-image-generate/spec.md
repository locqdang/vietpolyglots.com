...
- **FR-018**: A completed generation MUST show a **Load** action only. A non-completed generation with a `prompt_id` MUST show **Resume**, continue progress lookup by that same `prompt_id`, and MUST NOT create or rate-limit a duplicate generation.
