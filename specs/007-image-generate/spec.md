...
- **FR-016**: System MUST persist the gate `prompt_id` as soon as the GPU gate accepts a generation and expose it only to the owning user.
...
- **FR-018**: A completed generation MUST show a **Load** action only. A queued or processing generation with a `prompt_id` MUST show **Resume** and continue progress lookup by that same `prompt_id`. A failed generation MUST show **Try again**, create a new rate-limited generation request using the retained prompts, and replace the prior failed history record only after the new request is successfully queued. The user interface MUST NOT display internal prompt IDs or seed IDs.
...
- **FR-020**: The GPU gate MUST support asynchronous Chroma submission that immediately returns `prompt_id` and a status lookup endpoint keyed by `prompt_id`; application APIs MUST preserve owner checks before exposing status or results.
- **FR-021**: The progress area MUST display a `Queued → Generating → Success` stepper whose active step tracks the job stage (earlier steps shown done, later steps pending). The stage MUST reflect the GPU wait honestly: a job stays `Queued` (labelled "Waiting for the GPU…") until the gate acquires the GPU and returns a `prompt_id`, then advances to `Generating`, and all three steps MUST show done once the image completes. The stage and the progress bar MUST never move backwards.
