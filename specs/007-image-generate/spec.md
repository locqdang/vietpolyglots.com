...
- **FR-021**: The progress area MUST display a `Queued → Generating → Success` stepper whose active step tracks the job stage (earlier steps shown done, later steps pending). The stage MUST reflect the queue wait honestly: a job stays `Queued` (labelled "Waiting in queue…") until the gate acquires the GPU and returns a `prompt_id`, then advances to `Generating`, and all three steps MUST show done once the image completes. The stage and the progress bar MUST never move backwards.
