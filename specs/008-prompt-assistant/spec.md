...
- **FR-011**: System MUST show a loading/pending state on the assistant control while a request is in flight and prevent overlapping assistant requests. User-visible status copy MUST use plain language and MUST NOT expose internal technical terms (e.g. "GPU", "LLM", "model", "inference", "queue worker", "cohort"); internal status identifiers may be technical so long as they are never rendered.
